...
def extract_element(
    ...
    multiple = bool(element_cfg.get("multiple", False))
    limit = element_cfg.get("limit")
    try:
    ...
    for role, locator_cfg in locator_chain:
        ...
        try:
            ...
            continue
        try:
            WebDriverWait(driver, wait_timeout).until(condition((by, selector)))
            if multiple:
                ...
                for element in elements:
                    if attr == "text":
                        ...
                        values.append(element.get_attribute(attr))
                return values
