...
def _infer_fb_type_from_url(uid: str | None) -> str | None:
...
def _infer_module_from_crawl_types(
    crawl_types: Any,
    selector_modules: Dict[str, Dict[str, Any]],
) -> str | None:
...
def _infer_module_for_item(
    ...
    module = _infer_module_from_crawl_types(item.get("crawl_types"), selector_modules)
    if module:
        return module

    module = _infer_fb_type_from_url(item.get("uid"))
    if module and module in selector_modules:
        return module

    return _fallback_selector_module(selector_modules)
...
def main() -> int:
    ...
    for group in grouped_items.values():
        ...
        for module, module_items in module_buckets.items():
            uids = _collect_uids(module_items)
