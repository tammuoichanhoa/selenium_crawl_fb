...
def load_proxies(file_path: str) -> List[str]:
    if not os.path.exists(file_path):
        raise RuntimeError(f"Missing proxy file: {file_path}")
    ...
    if not proxies:
        raise RuntimeError(f"No proxies found in {file_path}")

    return proxies


def build_driver(user_agent: str, proxy: str) -> webdriver.Chrome:
    options = Options()
...
def test_proxy(proxy: str, user_agent: str, cookies: List[dict], index: int, total: int) -> bool:
    logging.info(
        "Starting test %s/%s with proxy: %s | user-agent: %s",
        index,
        total,
        proxy,
        user_agent,
    )
    print(f"[{index}/{total}] Testing proxy: {proxy}")
    print(f"  -> User-agent: {user_agent}")

    driver = build_driver(user_agent, proxy)
    try:
        ...
        if success:
            logging.info("SUCCESS | %s | %s", proxy, user_agent)
            print("  -> Success")
        else:
            logging.warning("FAILED | %s | %s", proxy, user_agent)
            print("  -> Failed")

        return success
    except Exception as exc:
        logging.exception("ERROR | %s | %s | %s", proxy, user_agent, exc)
        print(f"  -> Error: {exc}")
        return False
    finally:
        driver.quit()
...
def main() -> None:
    ...
    for index, proxy in enumerate(proxies, start=1):
        user_agent = user_agents[(index - 1) % len(user_agents)]
        if test_proxy(proxy, user_agent, cookies, index, len(proxies)):
            working_proxy = proxy
            break

    tested_count = index if proxies else 0
    if working_proxy:
        summary = (
            f"Found working proxy after {tested_count} test(s): {working_proxy}"
        )
    else:
        summary = f"No working proxy found after {tested_count} test(s)"
