...
1. **Tạo file `.env`** (ở cùng thư mục với mã nguồn) với các biến sau:
   ...
   - `LOGIN_METHOD`: `cookies` hoặc `profile`. Nếu bỏ trống sẽ dùng giá trị trong `config.yml` (mặc định `cookies`).
...
2. **Tùy chỉnh `config.yml`**:
   ...
   - Khối `crawl`:
     ...
     - `elements`: danh sách trường cần trích xuất với `name`, `by`, `selector`, `attribute`. Đây là phần bắt buộc.
...
2. Kích hoạt virtualenv (nếu có) rồi chạy:
   ...
   ```
3. Script sẽ:
   - Tạo ChromeDriver với proxy, user-agent và thư mục profile theo `.env`/`config.yml`.
...
1. Cập nhật `config.yml` và `pages.txt` cho đúng nhóm trang cần thu thập.
...
3. Chạy:
   ...
   ```
4. Ứng dụng sẽ:
   - Đọc `.env`, `config.yml`, chọn proxy đang hoạt động (`utils.select_working_proxy`).
   - Tạo tối đa `MAX_WORKERS` luồng (tự giới hạn khi chỉ có một profile hoặc dùng cookies).
   - Với mỗi URL, truy cập, chờ `wait_after_load` giây, rồi trích xuất các phần tử theo danh sách `crawl.elements` bằng Selenium.
...
Giữ README này cập nhật khi bạn thay đổi cấu trúc `.env`, `config.yml` hoặc thêm script mới để mọi người có thể chạy `login.py` và `crawler.py` dễ dàng.
