...
def build_driver(
    ...
    # (không mở cửa sổ trình duyệt, chạy nền - phù hợp khi deploy server, CI/CD...)
    if os.getenv("HEADLESS", "false").lower() == "true":
        options.add_argument("--headless=new")  # dùng headless mode mới của Chrome

    # Tắt flag AutomationControlled để hạn chế bị website phát hiện đang dùng automation (Selenium)
    options.add_argument("--disable-blink-features=AutomationControlled")

    # Mở trình duyệt ở chế độ full màn hình ngay từ đầu
    options.add_argument("--start-maximized")

    # Bỏ qua màn hình first-run khi mở Chrome lần đầu
    options.add_argument("--no-first-run")

    # Tắt popup yêu cầu đặt Chrome làm default browser
    options.add_argument("--no-default-browser-check")

    # Chỉ định thư mục chứa dữ liệu người dùng (cookies, localStorage, session...)
    # Giúp giữ đăng nhập, session giữa các lần chạy
    options.add_argument(f"--user-data-dir={profile_path}")

    # Chọn profile cụ thể trong thư mục user-data-dir (ví dụ: Default, Profile 1...)
    options.add_argument(f"--profile-directory={profile_name}")

    # Nếu có truyền custom User-Agent thì override User-Agent mặc định của Chrome
    ...
    # Lấy đường dẫn ChromeDriver từ biến môi trường
    # Giúp chủ động control version driver, tránh lệch version với Chrome
